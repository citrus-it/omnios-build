...
<!-- surveyed: omnios-build c0c15c106, illumos 1123bcf78e4, pkg5 bb55eef0ae2, kayak 23e2d98 -->
...
* The BIND DNS client utilities, such as [dig(1)](https://man.omnios.org/dig)
  ...
  [release notes](https://bind9.readthedocs.io/en/v9.20.27/notes.html) for
...
* The virtio framework now supports the event index feature, which reduces
  ...
  negotiated by the framework, so all virtio client drivers benefit. The
  `vioif` control queue implementation has also been extended in
  preparation for MAC filtering and promiscuous mode support.
...
* The system linker now supports the `-z sysroot` option, in a manner
  compatible with both the GNU and Oracle Solaris linkers. This constrains
  library and file search paths to an arbitrary root and is used by the GCC
  `--sysroot` option, easing cross-compilation.
